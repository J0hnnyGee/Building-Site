...
public class NuoveCredenzialiGUI extends JFrame {
	...
	private void aggiungiCredenziali(controller.Controller controller, String cf,  ArrayList<Login> lista) {
		...
		if (flag==0) {
			...
			JOptionPane.showMessageDialog(this, "WARNING! L'username scelto è già in uso.", "", JOptionPane.INFORMATION_MESSAGE);
	...
	public boolean check() {
 		if (! vuoto( username_field, "WARNING! Insert username."))
		    return false;
		  else
		  if (! vuoto( password_field, "WARNING! Insert password."))
		    return false;
		  else
			  return true;
	}
