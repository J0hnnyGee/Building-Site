...
public class CreaCantiereGUI extends JFrame {
	...
	public CreaCantiereGUI(controller.Controller controller, int id) {
				...
				JLabel data_label = new JLabel("Starting Date:");
				...
				JButton submit_button = new JButton("Foreman >>");
	...
	public boolean check() {
 		if (! vuoto( citta_field, "WARNING! Insert City."))
		    return false;
		  else
		  if (! indirizzo( via_field, "WARNING!  Check the address."))
		    return false;
		  else
		  if (! vuoto( proprietario_field, "WARNING! Insert the owner."))
			return false;
 		  if (! double_input( costo_field, "WARNING! Check the cost."))
			  return false;
 	  	  if (! data_vuoto( data_chooser, "WARNING! Insert starting date."))
			return false;
		  else
		    return true;	
}
	...
	public boolean indirizzo(JTextField f, String errore ) {
		String s=f.getText();
		if(!vuoto(f, "WARNING! Insert address."))
			return false;
	...
	{
		if(f.getDate()==null) {
			return fallitoData( f, "WARNING! Select the starting date." );
		}
		else return true;
	}
