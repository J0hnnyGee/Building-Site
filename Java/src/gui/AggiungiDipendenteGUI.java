...
public class AggiungiDipendenteGUI extends JFrame {
	...
	public AggiungiDipendenteGUI(controller.Controller controller,  String id) {
				//Grandezza finestra
				setSize(840,546);
				setResizable(false);
				...
				JLabel birth_label = new JLabel("Data Nascita:");
				...
				JLabel stipendio_label = new JLabel("Stipendio:");
				...
				ruolo_combo = new JComboBox();
				ruolo_combo.setModel(new DefaultComboBoxModel(new String[] {"Operaio", "Operatore"}));
				ruolo_combo.setToolTipText("");
				springLayout.putConstraint(SpringLayout.NORTH, ruolo_combo, 55, SpringLayout.SOUTH, cf_field);
				springLayout.putConstraint(SpringLayout.WEST, ruolo_combo, 0, SpringLayout.WEST, nome_field);
				springLayout.putConstraint(SpringLayout.EAST, ruolo_combo, 0, SpringLayout.EAST, nome_field);
				getContentPane().add(ruolo_combo);
				ruolo_combo.setFont(submit_font);
				
				JLabel ruolo_label = new JLabel("Ruolo:");
				springLayout.putConstraint(SpringLayout.WEST, ruolo_label, 0, SpringLayout.WEST, nome_label);
				springLayout.putConstraint(SpringLayout.SOUTH, ruolo_label, 0, SpringLayout.SOUTH, ruolo_combo);
				getContentPane().add(ruolo_label);
				ruolo_label.setFont(labels_font);
				
				JButton aggiungi_button = new JButton("Aggiungi");
				springLayout.putConstraint(SpringLayout.NORTH, aggiungi_button, 0, SpringLayout.NORTH, ruolo_combo);
				springLayout.putConstraint(SpringLayout.WEST, aggiungi_button, 0, SpringLayout.WEST, mail_field);
				springLayout.putConstraint(SpringLayout.SOUTH, aggiungi_button, -33, SpringLayout.SOUTH, getContentPane());
				springLayout.putConstraint(SpringLayout.EAST, aggiungi_button, 0, SpringLayout.EAST, mail_field);
				getContentPane().add(aggiungi_button);
				aggiungi_button.setFont(submit_font);
	...
	private void aggiungi(String id, controller.Controller controller) {
		...
		try {
				d.aggiungiDipendente(cf_field.getText(),nome_field.getText(), cognome_field.getText(), date_chooser.getDate(),  mail_field.getText(), telefono_field.getText(), indirizzo_field.getText(),
						 Double.parseDouble(stipendio_field.getText()),ruolo_combo.getSelectedItem().toString(), Integer.parseInt(id));
	...
	public boolean check() {
		 		if (! vuoto( nome_field, "ATTENZIONE! Inserire Nome."))
				    return false;
				  else
				  if (! vuoto( cognome_field, "ATTENZIONE! Inserire Cognome."))
				    return false;
				  else
				  if (! cf( cf_field, "ATTENZIONE! Il C.F. deve essere di 16 caratteri."))
				    return false;
				  else
				  if (! mail( mail_field, "ATTENZIONE! Controllare di aver inserito correttamente la e-mail."))
				   return false;
				  else
			       if (! telefono( telefono_field, "ATTENZIONE! Il numero di telefono deve contenere 10 cifre."))
				  return false;
				  else
			      if (! indirizzo( indirizzo_field, "ATTENZIONE! Controllare di aver inserito correttamente l'indirizzo."))
				  return false;
				  else
				  if (! double_input( stipendio_field, "ATTENZIONE! Controllare di aver inserito correttamente lo stipendio."))
				  return false;
				  else
				  if (! eta( date_chooser, "ATTENZIONE! Inserire dipendente che abbia almeno 16 anni."))
				  return false;
				  else
				    return true;
	...
	public boolean cf(JTextField f, String errore ) {
		...
		if(s.length()!=16) {
			...
			return fallito( f, "ATTENZIONE! Controllare che il C.F. contenga solamente lettere e numeri." );
	...
	String EMAIL_PATTERN = 
		...
		if(!vuoto(f, "ATTENZIONE! Inserire e-mail."))
		return false;
	...
	public boolean telefono(JTextField f, String errore ) {
		String s = f.getText();
		if(!vuoto(f, "ATTENZIONE! Inserire numero di telefono."))
			return false;
		if(s.length()!=10) {
			return fallito( f, errore );
	}
	...
	{
		if(f.getDate()==null) {
			return fallitoData( f, "ATTENZIONE! Selezionare data di nascita." );
		}
